...
IR::MethodCallExpression *gen_actioncall_expression(cstring method_name, IR::ParameterList params) {
...
IR::ActionList *actionList::gen(size_t len) {
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        IR::MethodCallExpression *mce = gen_actioncall_expression(act_name, *params);
        if (mce) {
